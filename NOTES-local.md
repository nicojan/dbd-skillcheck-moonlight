...
**2026-08-29 (22:50): armed matches can record themselves now — the "arm or record, one per session" rule is retired.** `autorun.py --record`. The rule was never about recording; it was about *two `mss` clients*, which mutually starve on a ~30 s timeout (the 2026-08-20 entry below: the recorder flatlined to exactly one frame every 30.0 s and the armed loop died with it). The armed loop already grabs a 672 box every frame and `grab_wide` already copies out of the mss buffer, so `dbd/utils/clip_recorder.py` keeps those frames rather than opening a second client. **Measured hot-loop cost: 0.21 us per frame, 0.001% of a ~31 ms loop** — it is a `deque.append`, and nothing is encoded until a frame is kept.
