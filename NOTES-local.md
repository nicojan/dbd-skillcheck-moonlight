...
**2026-09-03: matches played under external CPU load must not be scored, and one bout is already in question.** Stream judder chased all evening turned out to be an unrelated project's `next build` saturating the Mac — 123.8% CPU, a twelve-worker fan-out, WindowServer at 58-70%, and a **1-minute load average of 43.8 on 14 cores**. It is a data-integrity problem, not an operator annoyance: a contended scheduler inflates `round_trip_ms` and adds jitter that is indistinguishable from the link's in the landings log, and every constant here is fitted to those trips. **The 23:31-23:38 bout on 2026-09-02 overlaps the build window and carries a 479 ms stall in a stretch where the recorder wrote nothing — check its trip distribution against the 40-43 ms baseline before counting it toward the bias-3.0 evidence.** Run `uptime` before arming; above a load of ~6 nothing observed about the stream is about the stream. Full diagnosis, including the three wrong explanations that were checked first (the recorder, V-Sync, the link), is in `docs/audio-and-stream-tuning.md`.
