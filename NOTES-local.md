...
**What is left, in order of what the evidence supports.** (1) Link jitter, still the ceiling and still only movable by Moonlight/host settings or step 8 — which no longer has lost presses as a justification. (2) **The short-fit loss is no longer a finding** — pooled over all four matches it is 18/23 short against 99/113 long, a gap the sample cannot support. Do not chase it. (3) Nothing else — the fit, the sleep, the lead and the freeze watch are all closed, and the adaptive lead is now measured as a wash against a fixed 60 ms.
