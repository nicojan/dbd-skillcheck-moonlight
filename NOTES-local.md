...
1. ~~**Run the host game at 120 fps.**~~ **RULED OUT 2026-09-07 — the premise was false.** The operator confirms the host game has been at 120 fps since well before 09-03, so the whole 40-session record was already sampling input at 120 Hz and the 60 Hz term never existed. The residual game-polling term at 120 Hz is `8.3/sqrt(12)` = **2.4 ms**, and it is irreducible without going higher still. **This was never an open experiment; do not schedule it again.** The `sd_err` history confirms there is no step change to find: every clean session from 08-16 to 09-07 sits at `trip_sd` 10-13 ms and `sd_err` 3.3-5.9 deg, with 09-07 at 4.85 / 12.3 — the link got faster over that span (trip median 56-64 ms -> 40-48) and never got steadier.
...
**2026-09-13: the check is a PERK, and it is a 15-SECOND drumming Performance rather than a 2.90 s check. `1-2-3-4!` is Vee Boonyasak's, the Performance passes or fails as ONE unit, and the absence of visual feedback is the GAME'S, corroborated by players. The first key log exists and `tools/pair_keys.py` reads it back. My own "the press causes the relocation" reading from this morning is WITHDRAWN.**

**What it is, verbatim from the wiki.** *"A steady beat makes for steady work."* While standing and idle, press the Active Ability button to start a Performance: lasts for up to **15 seconds**, empowers Survivors within 16 metres, **triggers continuous Skill Checks**; after **successfully completing** the Performance empowered Survivors get +20% Skill Check chance for 90 s while healing or repairing; cool-down 110/100/90 s after **cancelling or completing**. It is a drumming rhythm mini-game — the perk has four song variations, and TheGamer's guide renders it as "play the drums for 15 seconds, during which there will be continuous skill checks". The sibling mechanic is **Bardic Inspiration**, same "Performance up to 15 seconds" and the same 110/100/90 cool-down, two years older and a second source of the same check type if one is ever needed.

**The coverage hypothesis is confirmed, and it is the perk's design rather than a quirk.** The buff is conditioned on *successfully completing* the Performance and the cool-down fires on *cancelling or* completing, so there is a pass/fail on the whole run and not on each check. A player on Steam describes the failure mode directly: "it just cancels it and counts it as failed skill check even tho the bar is literally inside the skillcheck". Every beat or nothing. That is exactly what the 21:00 match looked like — 3 of 5 arcs pressed, nothing at all on the last — and it needs no further hypothesis.

**"No visual indicator of success anywhere" is the game's, not our capture's.** Measured here on 09-12 against duration, arc count, arc width and the terminal freeze; independently complained about by players, who report the perk "doesn't show up like it's activated down in light blue color as usual" and "doesn't even give the red icon for increased skill check chance like you would get normally". Stop looking for one in the pixels. The keyboard is the only label this check has.

**The 2.90 s "check" is ONE BAR of four beats, and the unit that passes is the ~15 s Performance.** The beat period measured as the arc's "~679 ms relocation timer" is **~87 BPM**, and four beats at 87 BPM is 2.76 s against the 2.87-2.91 s measured — so what NOTES has been calling the check is one bar, which is what "1-2-3-4!" is named after. Today's key log is the direct evidence: burst of 5 presses over 2744 ms, a 3752 ms gap, then 11 presses over 7094 ms, **13.6 s from the first press to the last**, which is the 15-second Performance with the recorded "check" as its first bar. **This is the correction that matters most.** The tracker scores one bar, `ClipRecorder` clipped 4.2 s of a 13.6 s event, and no whole Performance has ever been recorded.

**What that does to the reliability bar.** Roughly 20 beats per Performance, every one of them required. At 95% per beat a Performance succeeds 36% of the time; at 99% per beat, 82%. Against a link carrying ~11 ms of jitter this is a far harder target than any ordinary check, and it is the number any future attempt has to be argued against — not the per-press landing accuracy, which was never the problem.

**The first calibration bout, and the two wrong ways to read it.** `frames/bout_20260913-130005` (`--dry-run --record-keys`, 123 frames, 16 presses, all `source: 1`) is the first hardware keypress ever read back, and it confirms the operator reads 1 where three injected bot presses read 0 — but it is a dry run, so nothing else could have pressed, and this is not a side-by-side. `tools/pair_keys.py` scores it at **3 of 4 presses on the arc, 2.0 / 9.5 / 12.0 deg past the leading edge**; the fourth reads 3.5 deg short of an arc whose clean leading edge the recording never caught, the clip starting two frames before that relocation. **Trap, and I fell in it exactly as the file warns:** scoring with `find_outline_arc` at the landing frame reported all five presses ~186 deg outside. `OUTLINE_MIN_DEG` throws the frame away precisely when the needle is inside the arc, so the gated read showed the needle on the arc in **zero of 123 frames** of a check where it makes 2.5 revolutions through a 110 deg arc — impossible, and that impossibility is what caught it. A constant offset is the shape a broken instrument makes, not the shape a wrong operator makes.

**WITHDRAWN, same day it was proposed: the arc relocation is not evidence of a registered press.** The four scorable presses are each followed by a relocation at 26-58 ms (mean 41, sd 12, against a link round trip whose recent median is 44), and against an independent 679 ms timer four lags under 60 ms has probability 6e-5. That looked causal. It is not safe to read that way: the arc relocates **on the beat of the music**, and an operator playing on the beat produces the same correlation with no feedback involved at all. The untouched 09-11 checks relocate on their own at the same cadence, and today's press intervals (649-703 ms) are indistinguishable from it. **The only thing that separates them is a Performance played deliberately OFF the beat** — hit the beats early, or skip one, and see whether the arc follows the press or holds the grid.

**Patch caveat on every recording before today.** `1-2-3-4!` was genuinely broken in **9.2.0** across Steam, PS5 and PC — the +20% did not apply at all, measured by players at 6.0 skill checks per gen against the 16.1 the buff implies — and was fixed in **9.2.1**. That bug is about the buff rather than the checks, but it means the game was not a clean instrument, and which patch each of the 09-11 and 09-12 recordings sits on is not currently recorded anywhere.

**Open, and the operator can answer it in a sentence.** The wiki's wording leaves it ambiguous whether the continuous Skill Checks go only to the performer or to every empowered Survivor within 16 m. It decides whether the bot can ever meet this check without a human pressing the Active Ability button first, which decides whether any of this is worth building.

**Scoring caveat on the whole of today.** Load was 18-48 for the entire session — the machine had been up 12 minutes and Spotlight was reindexing. Keystroke timestamps survive that; the frame clock they are paired against is exactly what load corrupts. One check, under load. Re-run clean before building on any number above.
...
**What the two matches actually established, and what they did not.** The 09-11 analysis of the check itself still holds and is below, unreverted: one ~111 deg unfilled arc, a ~679 ms relocation timer, the needle masking 6-12 deg of its own leading edge, the trimmed-tail fit at 1.8 deg RMS. What was never established, across both matches and every replay, is **whether a press placed inside the arc passes the check at all** — and that is the question the removal leaves open rather than answers. If this is ever picked up again, start there, with one deliberate manual press inside a drawn arc, before writing any detector.

**2026-09-12 (evening): the outline path was PLAYED and it failed, and the cause was not the aim — it was that the bot goes blind for 1.30 s after every press. Fixed in `0b3544f`, UNPLAYED.** The 16:35 match fired ten times on `1234!` checks and connected on none. Ordinary checks in the same match were fine: 32 fires, **21 GREAT / 10 good / 1 MISS**. The operator's report was three-part and all three are one mechanism: one check failed on the second revolution with no press at all, one press went out far too early, and the needle never stops for several revolutions.

**The measurement that settles it.** `report_landing` waits `FREEZE_WATCH_SECONDS` (0.80) for the needle to stop, because on an ordinary check a connecting press stops it dead and that freeze IS the landing measurement. This check's needle never stops, so the watch could only ever run to its deadline — and it did, on all ten fires, every one logging `still sweeping 800 ms after the press`. `HIT_COOLDOWN_SECONDS` (0.50) then slept on top and the tracker was dropped and re-acquired from zero. Fires came **1.5-2.2 s apart** against arcs that live ~650 ms and a revolution of ~1.2 s at 300 deg/s, so the bot missed a whole revolution of arcs after every press. **The freeze watch is a measurement instrument whose cost can exceed its value, and on this check it was the entire bug.**

**The aim was never the problem, and nearly got blamed anyway.** The detector reads the real arc — verified by eye against `frames/bout_20260912-163847`, where the bracketed outer arc on screen matches the logged `[outline arc 33-143 deg]` — and both fires that produced a measurable landing went inside it (202.5 into 187-318, 103.5 into 75-186). **Trap: the armed log's timestamps are second-resolution.** Reading the 16:44:19 fire as happening at t=332.0 made it look ~190 deg mis-aimed; the frames put the press later in that second and on target. Read frames before trusting a log timestamp to better than a second.

**What shipped.** Skip the freeze watch and the cooldown when `zone.outline`; keep the tracker live across the fire. That needed a new invariant, because the needle is sitting inside the arc it was just aimed at and would re-satisfy the "already inside" branch every frame for the ~370 ms it takes to cross a 111 deg arc — so `mark_fired` now retires the **arc**, keyed on its trailing edge (the identity `observe` already uses), instead of closing the check, and `decide` refuses an arc on that list. Ordinary checks are untouched: `fired_at_ms` still shuts them for good. Also fixed on the way: `no_press_note` read `fired_at_ms is None` as "never pressed" and would have filed a check the bot pressed seven times as `NO PRESS`; both call sites now go through `has_fired`.

**Two consequences for reading the logs.** Outline fires now log `landing: not watched` and carry **no verdict and no `error_deg` at all**, so they no longer pollute the landing-error line — the `-50.0 deg` floor in the 16:35 match summary was an outline fire scored against a Great centre this check does not draw. They also no longer feed the link-level round-trip estimate, which is a correctness gain rather than a loss: the one `round trip 33 ms` an outline fire produced came from a freeze on a check whose needle does not freeze, so it was almost certainly reading the check ending.

**Still open, and the reason this is not closed.** Whether a well-placed press connects on this check **at all** has never been tested, by anything. The replays only ever score the FIRST press of a check, so the multi-press path rests entirely on unit tests until a match is played. `tools/replay_outline.py` and `tools/replay_tracker.py` are byte-identical before and after the change (verified by stashing the diff and re-running), which proves no regression and proves nothing about the new path.

**2026-09-12: the `1234!` check is no longer abstained on. It draws a 111 deg UNFILLED arc that relocates on a ~679 ms timer, and the tracker now presses on it — 4 of 4 recorded checks land inside the drawn arc, against 0 of 4 before.** Four of these appeared in the 22:08 match on 09-11 (23:17:06, 23:23:01, 23:29:00, 23:32:14) and produced four `NO PRESS` lines and nothing else. They are the only four in the night's ~20k recorded frames; the 19:39 match had none. Exported to `recordings_1234/` (gitignored like every other recording set; the source frames are `frames/bout_20260911-225423`, 851 MB, **the only recording of this check type that exists — mark it keep before any `review_recordings.py` pass**).

**What it is.** ONE arc, 109-113 deg wide, drawn as the two thin rails of an ordinary GOOD zone — max fill run 2.0-2.2 px against `FILL_RUN_PX`'s 4.0 — with no solid band anywhere. `find_zone` returns None and `MIN_GREAT_DEG` refuses to invent a band, both correctly. The needle sweeps at **299-300 deg/s, constant, fitting at 1.73-1.87 deg RMS**; the arc relocates 4-5 times across the check's fixed 2.90 s. The classifier calls it `full black (out)` on 105 of 105 frames in three of the four and `repair-heal (out)` in the fourth — it has no class of its own, and does not need one, because tracking already starts on `full black (out)` (`TRACKED_PREDS`).

**The `fit too poor (15-16 deg RMS)` in the live log was never about the needle.** It is the FROZEN TAIL: the check runs 2.90 s against `MAX_SAMPLES`'s ~1.5 s, so by the end the fit window holds only the ~300 ms of stopped needle that follows the check. Trimmed the same way `ground_truth` trims it, the same frames fit at 1.8 deg RMS. Any future "the tracker cannot read this needle" claim has to survive that trim first.

**The arc's life is a TIMER and has nothing to do with the needle — this is the trap, and I fell in it.** The first two epochs checked by hand fitted "the arc relocates when the needle reaches its leading edge" beautifully, and I wrote that into the code as measured fact. Scored over all 14 epochs it is refuted outright: that rule predicts durations of 55-1178 ms against 508-760 observed, an **R^2 of -29.9 against a constant**. The arc lives 630 ms mean / sd 99 over all 14, and 679 / sd 55 over the eleven the recordings catch whole. **Two hand-checked cases are not a measurement.**

**The policy follows from the timer, and two wrong versions are worth keeping.** Because the arc expires on its own, the press has to be the SOONEST landing inside it, never the best-placed one. (a) Aiming at the leading edge and scheduling the wait put all four presses onto arcs that had already relocated — the needle needs ~600 ms on average to reach an arc it is not already inside, which is the whole of an arc's life. (b) Gating that wait on a modelled remaining life declined all four, including presses onto arcs that were VISIBLY still drawn. What ships models nothing: `decide` presses only on a frame where the needle is ALREADY inside the arc that frame shows, so the exposure is exactly one round trip — ~7% at a 679 ms life, and irreducible.

**The needle masks the arc it is standing on, and that cost two more wrong answers.** `static_image` blanks anything redder than `NEEDLE_REDNESS`, and the needle has a glow, so the arc's LEADING edge reads 6-12 deg late once the needle arrives — check_001's arc reads 130 while the needle approaches and 148 with the needle at 142. Keying the arc's identity on that edge made `decide` chase an edge that ran away from the needle exactly as fast as the needle chased it, so the "already inside" branch never once fired. The arc is now identified by its **trailing** edge, which the needle has not reached, and the zone kept is the one read when the arc was ACQUIRED — the last clean look at where it starts.

**Merciless Storm is untouched and the guard is tested.** Storm draws the same unfilled outline at 39-40 deg (`recordings_video/merciless-storm`, 13-40 across every frame) and Madness Storm at 37-56; `1234!` never reads below 106. `OUTLINE_MIN_DEG = 90` separates them with 34 deg of daylight, and a three-frame stability rule discards Madness Storm's isolated wide reads. Replayed: **16 of 16 and 13 of 13 still NO FIRE**, and `recordings_missed`, `recordings` and `oppression` are byte-identical to before the change. The withdrawn Storm abstain recommendation is not affected by any of this.

**Two bugs the tests caught, both in the shipped guard rather than the analysis.** `_max_contiguous` already returns pixels; scaling it by `RADIUS_STEP` again made the solid-band guard 4x too lenient and let a solid 8 px band read as an outline. Replacing that with `.any()` over filled angles then refused every real check, because compression speckle puts 3-4 scattered angles of an outline arc over `FILL_RUN_PX` out of 360. The guard is now the exact complement of `find_zone`'s Great-band test — a contiguous angular RUN of filled angles, `>= MIN_GREAT_DEG`.

**To test it, follow `docs/testing-the-1234-check.md`** — the operator-facing protocol: dry run first, then armed, what each log line means, and the one thing only the operator can report (whether the check actually passed in the game).

**Evidence, and what is NOT evidence.** `tools/replay_outline.py` scores these; `replay_tracker.py` structurally cannot, because it reads one zone from the static median of a whole check and there are four different arcs in it (its verdict on `recordings_1234` is UNSCORED, correctly). At the link's measured level of 46 ms all four land **9.7-19.9 deg inside a ~111 deg arc**, with 90-103 deg of margin to the trailing edge, and they hold **4 of 4 across a true round trip of 21-65 ms — 94.5% of the 163 trips measured on 09-11.** Above 75 ms they fail. **Nothing here has been played.** Whether a press inside the arc actually passes the check is unverified by construction: nobody pressed in any of the four recordings, so there is no recorded success or failure to calibrate against, and the arc's relocation is a timer rather than a response to anything. The live tell is in the log — a fire now prints `[outline arc N-N deg]` and the landing record carries `"outline": true` — and `score_freeze` reports it `ungraded` inside the arc and `MISS` outside, which is the honest pair.

**2026-09-03: the load gate is now enforced at launch, and the 09-02 contamination scare was ONE FIRE, not a bout.** Stream judder chased all evening on 09-02 turned out to be two concurrent Claude sessions under `~/dev/human` saturating the Mac — session `2939f3a6` running `npm run build` in human-dashboard at 23:58 and 00:11, session `d443bbcf` running the human-lesson-builder pytest suite across the same minutes — for a **1-minute load average of 43.8 on 14 cores**. Two things are now settled that were open.

**The 23:31-23:38 bout is CLEAN and counts.** Its 19 predictive fires sit on a frame-age median of 22.0 ms (every clean session: 20.8-23.7) and a tail-read median of 80.3 ms (every clean session: 76.6-80.0) — no shift, no fat tail. Exactly one fire is bad, 23:37:28, and it is bad on the independent wall clock: tail read **184.4 ms against a hard ceiling of ~87**, which is the 479 ms stall. It was being scored as a real +43 deg landing because at a trip of 181.0 it slipped under `MAX_PLAUSIBLE_TRIP_MS = 200`. `rescore_policy.py` now also gates on `MAX_TAIL_READ_MS = 120`; it excludes that fire and, across all 546 recorded fires, no other.

**`round_trip_ms` is NOT independent evidence about the link, and this is the trap to hold on to.** It is derived from the settled angle (`autorun.py`, the `time_to_angle` call), so it and `error_deg` are one measurement wearing two hats: reconstructing either from the other is exact to a **median 0.01 deg over 546 fires**. A miss on a fast trip therefore says only "the press landed early" — it is not evidence of link jitter, load, or anything else. `tail_read_ms` and `frame_age_ms` are the only wall-clock readings and are the only ones that can distinguish a stalled machine from a slow link.

**The `dbd` launch now warns above a load of 6** (`DBD_LOAD_GATE` to override), naming the load, the core count and the top three CPU consumers, and telling you not to score the match. It **warns and plays on** — a busy machine is a fine evening to play and only a bad evening to score, and refusing the launch would trade a real match for a data point. It fails open on any unreadable or malformed reading, and `tools/test_dbd_shell.py` pins that with six tests plus a negative control against a mutant that blocks. Full diagnosis of the original incident, including the three wrong explanations checked first (the recorder, V-Sync, the link), is in `docs/audio-and-stream-tuning.md`.

**The `--record` ring costs memory, not cadence — measured 2026-09-03.** The earlier measurement compared encoding and I/O with the ring running in both arms, so the append itself was never in the delta. Isolated (recorder present and quiet vs `recorder is None`, 4000 iterations of 672x672x3 frames): `offer()` costs **0.25 us median, p99 0.46, worst 12.4** against 0.04 for the no-op — 8 parts per million of a 25 ms frame budget, so it cannot touch loop cadence. What it does cost is **156 MB of resident memory** (peak RSS 215.9 MB against 59.5), holding 120 live frames at the 3.0 s pre-roll; the 240-frame cap would make that 325 MB if the loop ever ran at 80 fps.

**Updated 2026-09-01 (late). `--seed-lead` RAN FOR THE FIRST TIME, it works, and the aim came down to 3.0 because of it.** The 22:41 match opened with `cold start: seeded at 37 ms from the last run` — fire #1 took lead 36.8 instead of the 60 ms constant against a 49 ms trip and landed **+7.5 deg LATE, a good**, where the same fire had MISSED early in each of the two sessions before it. Match: **47 fires, 25 GREAT / 11 good / 2 MISS** over 38 graded, mean error +2.89, sigma 4.48, trip median 44.7. Both misses are mid-session bursts (trip 28.1 and 32.7 against lead 60), which is the case the burst rule structurally cannot catch — it fires on the SECOND check of a run.

**Why the seed had never run before, and it was not the code.** Two earlier matches that day (14:11 and 15:36) were launched from a shell holding a STALE copy of the `dbd` function: the `%Y%m%d-%H%M` log-name fix landed 08-30 and `--seed-lead` was added late on 08-31, so a shell opened between those dates produces correctly-named armed logs while silently dropping the flag. The tell is at shutdown — `link level N ms stored for the next run (enable with --seed-lead)`; `autorun.py:1218` only appends that suffix when `args.seed_lead` is false. **After editing the `dbd` function, `source ~/.zshrc` or the running shell keeps the old one.** Check with `functions dbd | grep -c seed-lead` (NOT `type dbd` — zsh's `type` prints only where a function lives, never its body, so that form returns 0 whether the flag is there or not and can never pass). **Corrected 2026-09-12: the `type dbd` form recorded here from 09-01 was itself broken** — it returned 0 on a shell whose `dbd` demonstrably had the flag, so it never once tested what it claimed to.

**`AIM_BIAS_DEG` 3.5 -> 3.0, and it reverses the 08-31 call for a reason.** That table scored 3.0 as regressing 5 of 30 sessions, but it was built against a seed that had never executed. Re-scored over **1229 gradeable fires across 33 sessions** with `rp.seeded_cold_only` + `previous_seed`: 3.5+seed gives 856G/11M whole and 275G/5M on the current link; **3.0+seed gives 896G/12M whole and 287G/4M on the current link** — +12 Greats AND one fewer miss where it counts. The whole-record row shows one MORE miss, and that is the entire case against; both regressed sessions (`260818-162457`, `260819-194107`) are pre-08-20, from the ~61 ms link that no longer exists. **Across the last 12 sessions 3.0 regresses nothing — zero sessions with more misses, zero with fewer Greats.** 2.5 is the floor (6 regressions, 17 misses).

**2026-09-07 (late): moving `ROUND_TRIP_MS` was proposed AGAIN on a fresh 159-fire evening, and ruled out AGAIN by the same trap. It stays 60.** Three matches on 09-07 (16:18, 19:25, 21:16) gave **159 graded fires, 117 GREAT (74%), 5 MISS (3.1%)** from the logged verdicts — on-model against the 42-session record. All five misses share one shape: each is the **FIRST fire after the link snapped fast**, taken at a lead of 60 or above. Preceding trip -> miss trip: 64.9->41.2, 64.4->22.0, 45.0->29.1, 43.0->29.0, 50.8->25.1. **Zero misses in the 122 fires that evening where the lead was under 60.** Landing error tracks round trip at r=+0.80 with slope 0.23 deg/ms, which is just the needle rate — the press lands where the trip puts it.

That mechanism is real, and it is NOT what the burst rule catches (it fires on the second check of a run) nor what `--seed-lead` catches (`cold_start_base` applies only while trips < `LEVEL_MIN_SAMPLES`). It happens **mid-session**, when the rolling median-of-9 climbs back inside the deadband and `lead_level_ms` snaps the base lead to the 60 ms constant on a link now running at 31 ms.

**Why it still does not justify moving the constant, and this is the trap to hold on to.** Scored UNSEEDED the case looks overwhelming — base 50 gives 745G/16M against base 60's 738G/32M on the 18 sessions of the current link, i.e. more Greats AND half the misses. **Scored SEEDED, which is the configuration that actually runs, the gain nearly vanishes: 743G/16M against 741G/20M — two Greats and four misses over 1012 fires.** The unseeded comparison overstates the miss benefit fourfold because most of what a lower anchor "fixes" is the cold start, which the seed already fixes at the source. This is exactly the error the 09-01 entry above warns about, made again with a bigger sample and a real mechanism attached, which is why the warning is worth keeping: **a mechanism you can see in the logs is not evidence that a given knob is the lever for it.**

**And it fails the per-session standard.** Seeded, base 50 makes 5 sessions better and **4 sessions worse**, adding a miss to sessions that previously had zero (`260824-194829` 0->1, `260824-204402` 0->2, `260829-144354` 0->1, `260903-205440` 0->1). No anchor tested avoids this: base 58 is 1 better / 4 worse, base 55 is 2/3, base 52 is 6/4, base 50 is 5/4. A policy that wins overall by losing a session is not an improvement.

**Two further things measured the same evening and rejected.** (1) **An adaptive/EWMA lead chasing the measured round trip** — worse at every gain from 0.1 to 1.0 on n=89. Lag-1 autocorrelation of round trip is only **+0.39** against a landing error of sd 5.3 deg, so chasing it adds more noise than it removes. Third independent confirmation of the standing autocorrelation result. (2) **Lowering `LEVEL_MIN_MS` below 30.** Worth checking only if the anchor moves: at base 50 the tracker's active band collapses to a median-of-9 in (30, 35) and the floor binds on 38% of windows. Swept 30/27/25/22/20, **misses stay at exactly 16 at every value** and Greats fall 745 -> 731. The floor stays at 30 regardless.

**What the evening leaves open is the instrument, not the policy.** Round trip ranged 12.1 to 66.7 ms within the single 21:16 session, and `error_deg` is 65-80% explained by it. That spread is next step 1 (link jitter) and step 8 (host-side `SendInput`), which is where the remaining misses live. **Caveat on all of the above: load ran 5.7-7.1 all evening, over the gate.** The miss mechanism is visible in the trips themselves so it is not load-created, but the absolute rates are softer than they look, and no idle match has been played since.

**Ruled out the same evening: moving `ROUND_TRIP_MS`.** Scored WITHOUT the seed, the stale 60 ms constant looks like the problem and seven configs dominate it (best `base 56`). Scored WITH the seed the lead constant stops mattering — `base 60 bias 3.0` (+12G/-1M) beats `base 56 bias 3.0` (+6G/-1M). The lead mismatch was a cold-start symptom and the seed treats it at the source. **Do not move the lead constant on an unseeded re-score.**

**Also ruled out: the cold-start "probe" (deliberately aiming late on fire #1 to buy a link measurement).** Measured and it worked — fire #1 misses in 6 of 31 sessions (19.4%, against ~3% session-wide), and a +4 deg probe took that to zero for a net 3 Greats. It is unnecessary: the seed fixes the same failure by centring the shot instead of sacrificing it. Kept here because the measurement is real if the seed ever proves unreliable.

**All six remaining misses in the check log are EARLY, zero late, every one on a 9-33 ms trip** (`pull_check_stats.py --peek`, 108 GREAT / 52 good / 6 MISS over 8 bouts). The late-miss mode introduced at 4.5 has not recurred, and moving down to 3.0 can only shrink it.
...
**What the surviving log says.** Eight armed stretches over 75 minutes, **28-37 fps** against the ~32 predicted for wide capture, so the 672 box costs nothing visible at real fps. **105 predictive fires, 98 graded: 68 GREAT (69%), 28 good, 2 MISS**, plus 32 reactive hits. **Both misses are EARLY**, and the check log names the cause of the big one: the -42.5 deg at 23:37:05 came back with a **round trip of 1071 ms** — a link stall three orders of magnitude off the 48 ms median, not an aim or a fit problem. The other is -7.0 at a 24 ms trip, the ordinary fast-link case the burst rule exists for. So a second full session at 4.5 deg bias has produced **zero late misses**, and the pulled queue over 2026-08-24 to 08-30 says the same at a larger n: **186 graded, 124 GREAT (66.7%), 59 good, 3 MISS — 3 early, 0 late.** Four bouts recorded, **141 checks, 993 MB in 70 minutes** — an eighth of the 7.4 GB/hour a continuous recorder would have cost, which is the ring buffer doing exactly what it was built for.
...
**2026-08-30 (10:15): `dbd` now says at launch when bouts are waiting — a notice, never a prompt.** The question was why the review TUI runs *after* the match rather than with the game, and the answer is that the TUI's only job is "which of these was worth keeping", which is knowledge that decays in about an hour: run at launch it would be reviewing the *previous* night's bouts, cold, and since everything starts unchecked, a review you cannot answer keeps nothing. Launch is also the worst moment to hold the terminal — the wait is there so the stream can settle and the very next action is focusing Moonlight, which the bot is blocking on. **But the after-run TUI is the ONLY chance those frames ever get**, and a run that never reaches it leaves them on a 92% full volume with nothing ever mentioning them again — which is exactly how four bouts and 993 MB sat unnoticed. So launch gets `review_recordings.py --pending`: one line when something is waiting, **complete silence when nothing is**, because the normal evening has nothing pending and a line printed every time is a line nobody reads by the third one. Two things worth keeping from building it. The block does **not** discard stderr, because the `dbd` preflight's own rule is that a check which fails silently is worse than no check — a broken notice reads as "nothing is waiting" forever, which is the failure it was added to end; it warns and plays on instead, and a test pins that the launch is never blocked. And the pty suite grew a general `extract(start, end)` so a second block of the real function could be tested without copying it.
...
**The check queue was drained on 2026-08-30 and this is how: `pull_check_stats.py` treats ANY argument that is not `--peek` as a pull, `--help` included.** There is no argparse in it; `main` reads `"--peek" in argv[1:]` and does the full pull otherwise, so `--help` printed the report AND moved 49 bouts to `checks/archive/`. Nothing was lost — draining is a move and `rescore_policy.py` reads the whole archive — but a queue that was waiting to be looked at was consumed by a command that asks for help. **Use `--peek` for anything exploratory**, and consider giving the tool a real `-h`. The report it produced, for the record: 270 checks over 49 bouts, 198 predictive / 49 reactive / 23 no-press (17 of those "only 4 samples"), 186 graded at 124 GREAT (66.7%) / 59 good / 3 MISS, all three early, median trip 48 ms over 198 measured.
...
~~**Unverified: none of it has been played.**~~ **PLAYED 2026-08-30** — 141 checks in four bouts, 993 MB over 70 minutes, bout boundaries landing where matches did. **The drop count is still unmeasured**, because Ctrl-C killed `tee` before the shutdown block could write it; see the 00:40 entry at the top of *Resume here*. It remains the first thing to read on the next armed run.
...
**Partly played 2026-08-30: the wide grab ran a full armed match (105 predictive fires, 28-37 fps, no regression) but NO DOCTOR was in it**, so every off-centre result below is still a replay and the questions are unchanged. Only `repair-heal` and `full white` were classified all night. The sweep now reports itself at shutdown (see the 02:30 entry at the top of *Resume here*), so the next match will at least say whether it fired. Watch on the next armed Doctor match: whether SNAP OUT OF IT checks appear in `checks/` at all, the loop's fps (predicted ~32 against ~34.5), and whether any off-centre press lands — a Madness check freezes where it was drawn, so `report_landing` now reads the held window rather than the centre crop, and that path has never run against a real freeze.
...
**~~Still open: the 120 fps experiment.~~ CLOSED 2026-09-07, and it was never open.** This line assumed the host game was at 60; the operator confirms it has run at 120 since well before 09-03. The "largest identified jitter term" was therefore already absent from every match in the record, which is why no amount of tuning has moved `trip_sd` off 10-13 ms. **Discriminator #2 (Moonlight bitrate/decoder) is now the cheapest remaining experiment** and the one that decides whether step 8 is justified.
