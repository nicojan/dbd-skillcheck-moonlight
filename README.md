...
**Off-centre Doctor checks are handled as of 2026-08-29, and the fix has been played but not yet exercised.** The Doctor's Madness displaces a check from dead centre, so the fixed 224 crop never captured it: a silent miss no lead or aim value could reach. One recorded Doctor match holds **nine of them, six being SNAP OUT OF IT** — an action that exists only against a Doctor and that this bot had never seen. `dbd/utils/wide_capture.py` grabs a 672-pixel box instead, classifies its centre 224 slice (byte-for-byte the frame the old path grabbed), and only when that slice is empty sweeps the box for the check's ring and re-crops about it. Replayed over those nine it presses **9 of 9** against the old path's 1, and its decisions on the same match's 58 centred checks are byte-identical. It then ran a full armed match on 2026-08-30 — 105 predictive fires, no regression, 28-37 fps — but **no Doctor was in it**, so the off-centre path itself is still untested live. `--no-wide` reverts it. The sweep also **reports itself at shutdown** — frames swept, rings proposed, locks taken, frames read off-centre — because nothing else in the log distinguishes a re-centred crop from a centred one, and a run where it never fired says so out loud.

**An armed match records itself as of 2026-08-29, played 2026-08-30.** Until then it was arm *or* record — not because recording was expensive, but because a second `mss` client starves the first on macOS. The armed loop already grabs the 672 box, so `dbd/utils/clip_recorder.py` keeps those frames instead of opening a second client: **0.21 us per frame**, 0.001% of the loop. A ring buffer writes nothing until a check fires, so only the checks and their lead-in reach disk. `tools/review_recordings.py` then tallies the recorded bouts with **everything unchecked**, and `dbd` opens it when the run ends. Rare perks and killer powers were previously unrecoverable after the fact; now they are not. First armed run: **141 checks in four bouts, 993 MB over 70 minutes** — an eighth of what continuous capture would have cost.
...
**BACKSPACE disarms and re-arms the bot mid-match**, without stopping the run. Disarmed it presses nothing, writes no landing and queues no check — but the classifier keeps reading and `--record` keeps writing frames, so a check you play by hand still leaves footage behind. Use it for anything the bot is worse at than you are: Merciless Storm draws no solid band, so the zone read degenerates and it aims at a Great centre it never measured. The toggle only counts while the game window is frontmost — a backspace typed in a terminal or a chat window is ignored — and every flip prints an `ARMED` / `DISARMED` line, because a run that is quietly doing nothing looks exactly like a quiet match. It needs the same Input Monitoring grant as the `1-2-3-4!` guard; if the tap fails to start, the run says so and the toggle is off.
...
# CAUTION: any argument that is not --peek is a pull — `--help` drains the queue too.
```
...
**Played 2026-08-30, and the one number it was played for is still missing.** 141 checks in four bouts, 993 MB over 70 minutes, no sign of trouble — but Ctrl-C killed `tee` before `autorun`'s shutdown block could write, so `recorder.summary()` never reached the terminal or the log. That is fixed (see *Ctrl-C is the supported way to end a run* above), and the **drop count in the recorder's shutdown line** is still the first thing to read on the next armed run: a non-zero drop means the writer threads could not keep up with a real disk and frames were thrown away rather than written. No replay or test can reach it.
...
| `test_wide_capture.py` | tests the wide box's geometry at four content heights, all three crop-lock rules, the loop wiring, and the sweep's own shutdown tally |
