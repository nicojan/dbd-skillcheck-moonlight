...
def regrade(sessions, lead_of, bias_deg=AIM_BIAS_DEG):
    """Grade every fire under `lead_of(last_trip_ms)` and an aim bias, via score_freeze.
...
def main(argv):
    ...
    fixed = regrade(sessions, lambda _t: base)
    print(line("fixed %.0f ms (the floor to beat)" % base, *fixed))
    print(line("SHIPPED: burst rule, <%.0f -> %.0f"
               % (autorun.BURST_TRIP_MS, autorun.BURST_LEAD_MS),
               *regrade(sessions, lambda t: autorun.lead_for_check(base, t))))
    for lead in (56, 58, 62, 64):
        print(line("fixed %d ms" % lead, *regrade(sessions, lambda _t, l=lead: float(l))))
    ...
    for name, fires in sessions:
        one = [(name, fires)]
        fv, _ = regrade(one, lambda _t: base)
